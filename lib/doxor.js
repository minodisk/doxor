        ...
        return Deferred
          .loop(comments.length, function (j) {
            ...
            if (comment.description && comment.description.body) {
              return Deferred
                .parallel([
                highlightCode(comment.description.body),
                highlight(comment.definition)
              ])
                .next(function (results) {
                  comment.description.body = results[0];
                  comment.description.full = comment.description.summary + comment.description.body;
                  comment.definition = results[1];
                });
            }
          })
          .next(function () {
  ...
  function copyFile(input, output) {
    ...
    fs.stat(input, function (err) {
      ...
      util.pump(is, os, function (err) {
        ...
        util.log('copy file: ' + input + '->' + output);
        dfd.call();
      });
    });
